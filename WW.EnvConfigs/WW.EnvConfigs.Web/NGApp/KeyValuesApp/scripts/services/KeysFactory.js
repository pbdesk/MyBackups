...
(function () {
    ...
    angular.module(KeyValuesAppName).factory(serviceId, ["$http", "$q", KeysFactory]);
    ...
    function KeysFactory($http, $q) {
        ...
        var _getKeys = function (id, hardRefresh) {

            var deferred = $q.defer();

            var cacheKey = "KeysCache" + id;
            if (sessionStorage.getItem(cacheKey) && !hardRefresh) {
                angular.copy(angular.fromJson(sessionStorage.getItem(cacheKey)), _keys);
                _isInit = true;
                ...
                .then(
                    function (result) {
                        ...
                        $.each(res, function (idx, val) {
                            if (val.WWFramework) {                               
                                val.WWFramework.EnvKeys = null;
                                val.WWFramework.WWApps = null;
                            }
                        });

                        angular.copy(res, _keys);
                        sessionStorage.setItem(cacheKey, angular.toJson(_keys));
                        _isInit = true;
                        deferred.resolve(_keys);
                    },
                    function () {
