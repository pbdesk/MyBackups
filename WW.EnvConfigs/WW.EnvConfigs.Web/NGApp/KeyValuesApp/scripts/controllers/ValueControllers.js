...
var EditValuesController = function ($scope, $routeParams, KeyValueFactory, DirtyFactory) {
    ...
    $scope.ApplyValueToAllFilteredItems = function (valueOfKey, filteredItemsArr) {
        var r = confirm("Value \"" + valueOfKey + "\" will be applied to all currently filtered " + filteredItemsArr.length + " items. Confirm?");
        if (r == true) {
            KeyValueFactory.ApplyValueToAllFilteredItems(valueOfKey, filteredItemsArr);
        }
    }
